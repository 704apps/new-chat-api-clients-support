...
function setupSocketIO() {
    ...
    server_1.io.on("connection", function (socket) {
        ...
        socket.on("answerCall", function (data) {
            var socketId = data.projectId;
            if (socketId) {
                server_1.io.to(socketId).emit("returnCall", data.signal);
            }
        });
        ...
        socket.on("callUserSupport", function (data) { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                try {
                    ...
                    dataCall = __assign(__assign({}, data), { signal: data.signal, from: data.from });
                    if (socketId) {
                        server_1.io.to(socketId).emit("callUserClient", dataCall);
                    }
                }
                catch (error) {
                    throw new AppError_1.AppError('Unexpected error', 400, { error: error });
